...
private:
	...
	BOOL CopyBinaryToRemoteSystem()
	{
		...
		_stprintf(szRemoteResource, _T("%s\\ADMIN$\\System32\\%s%s"), m_lpszMachine, fname, ext);
	...
	BOOL ExtractLocalBinaryResource()
	{
		...
		_stprintf(m_szLocalBinPath, _T("%s\\%s"), m_szLocalBinPath, ProcComs);
	...
	BOOL CopyServiceToRemoteMachine()
	{
		...
		_stprintf(szSvcExePath, _T("%s\\ADMIN$\\System32\\%s"), m_lpszMachine, RemComSVCEXE);
	...
	BOOL ConnectToRemoteService(DWORD dwRetry, DWORD dwRetryTimeOut)
	{
		...
		_stprintf(szPipeName, _T("%s\\pipe\\%s"), m_lpszMachine, RemComCOMM);
	...
	BOOL BuildMessageStructure(RemComMessage* pMsg)
	{
		...
		// Cmd
		if (!IsCmdLineParameter(_T("c")))
			_stprintf(pMsg->szCommand, _T("%s %s"), m_lpszCommandExe, m_szArguments);
		else
		{
			TCHAR drive[_MAX_DRIVE];
			TCHAR dir[_MAX_DIR];
			...
			_stprintf(pMsg->szCommand, _T("%s%s %s"), fname, ext, m_szArguments);
		}
		...
		// Console Title
		_stprintf(m_szConsoleTitle, _T("%s : %s"), m_lpszMachine, pMsg->szCommand);
	...
	void ListenToRemoteNamedPipes()
	{
		...
		std::thread([=](void* p) 
		{
			this->ListenRemoteOutPipeThread(p);
		}, this);
		...
		std::thread([=](void *p)
		{
			this->ListenRemoteStdInputPipeThread(p);
		}, this);
	}
...
public:
	int Run()
	{
		...
		_stprintf(m_szConsoleTitle, _T("%s : Starting Connection to: "), m_lpszMachine);
		SetConsoleTitle(m_szConsoleTitle);
		...
		if ((_tcsnicmp(m_lpszMachine, m_lpszLocalMachine, 16) == 0) || (_tcsnicmp(m_lpszMachine, m_lpszLocalIP, 16) == 0) || (_tcsnicmp(m_lpszMachine, ".", 2) == 0))
		{
			...
			if (ExtractLocalBinaryResource()) {
				...
				_stprintf(szExeCmdAsUser, _T("%s %s %s %s"), m_szLocalBinPath, m_lpszUser, m_lpszPassword, m_lpszCommandExe);
