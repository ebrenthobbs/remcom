...
class RemComMessage:
	...
	def setCommand(command):
		...
		self.__payload['CommandLength'] = command.length()

	def getLogonFlags():
		return self.__payload['LogonFlags']

	def setLogonFlags(logonFlags):
	...
	def receive(pipe):
		...
		if (!self.__receiveHeader(pipe))
			return False;

		if (!self.__writeAck(pipe))
			return False;

		if (!self.__receiveCommandText(pipe))
			return False;
	...
	def send(pipe):
		...
		if (!self.__sendHeader(pipe))
			return False;

		if (!self.__readAck(pipe))
			return False;

		if (!self.__sendCommandText(pipe, command))
			return False;

		return self.__readAck(pipe)

	def createPipeName(baseName, pipeName):
		return "\\\\.\\pipe\\" + baseName + self.__payload['Machine'] + self.__payload['ProcessId']

	#
	# Private
	#

	def __receiveCommandText(pipe):
		commandText = self.__readBytes(pipe, self.__payload['CommandLength'], "command bytes")
		self.setCommand(commandText)
		return True

	def __sendCommandText(pipe):
		return self.__writeBytes(pipe, self.command, self.__payload['CommandLength'], "command bytes")

	def __receiveHeader(pipe):
		bytes = self.__readBytes(pipe, sys.getsizeof(self.__payload), "header bytes")
		self.__payload = RemComMessagePayload(bytes)
		return True

	def __sendHeader(pipe):
		return self.__writeBytes(pipe, self.__payload, sys.getsizeof(self.__payload), "header bytes")

	def __readAck(pipe):
		response = RemComResponse;
		bytes = self.__readBytes(pipe, sys.getsizeof(response), "ack bytes")
		response = RemComReponse(bytes)
	...
	def __writeAck(pipe):
		...
		return self.__writeBytes(pipe, response, sizeof(response), "ack bytes")
	
	def __readBytes(pipe, bytesToRead, suffix):
		totalBytesRead = 0
		buf = ""
		while totalBytesRead < bytesToRead:
			bytesRead = 0
			bytes = s.readNamedPipe(pipe.tid, pipe.fid_main, bytesToRead)
			if bytes > 0:
	...
	def __writeBytes(pipe, bytes, bytesToWrite, suffix):
		totalBytesWritten = 0
		curPtr = bytes
		while totalBytesWritten < 0:
			bytesToWriteThisTime = bytesToWrite - totalBytesWritten
			if bytesToWriteThisTime < self.__readBufferSize:
				bytesToWriteThisTime = self.__readBufferSize
			bytesWritten = 0
			bytesWritten = s.writeNamedPipe(pipe.tid, pipe.fid_main, str(curPtr), bytesToWriteThisTime)
...
# The first parameter here tells the check whether or not to start the service if it exists.
            ...
            message = RemComMessage()
            pid = os.getpid()

            message.setMachine(''.join([random.choice(string.letters) for _ in range(4)]))
